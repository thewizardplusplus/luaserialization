...
function Object.from_options(options, use_exception)
  ...
  if id_as_string == nil then
    return _handle_error("the `name` option has an invalid format", use_exception)
  end

  local id = tonumber(id_as_string)
  if id == nil then
    return _handle_error("unable to extract an ID from the `name` option", use_exception)
  end
...
function ObjectWrapper.from_options(options, use_exception)
  ...
  if not checks.has_properties(options.object, {"id"}) then
    return _handle_error("the `id` property of the `object` option is missing", use_exception)
  end
...
for _, data in ipairs({
  ...
  },
  {
    ...
    args = {
      text = [[{"one":{"__name":"ObjectWrapper","object":{"__name":"Object","name":"test-23"}}}]],
      constructors = {
  ...
  },
  {
    ...
    want = nil,
    want_err = "^unable to apply the constructors: unable to apply the constructors: unable to call the constructor: error: the `name` option has an invalid format",
  },
  {
    ...
    want = nil,
    want_err = "^unable to apply the constructors: unable to apply the constructors: unable to call the constructor: .+: exception: the `name` option has an invalid format",
  },
}) do
  TestJson[data.name] = function()
    local result, err = json_module.from_json(data.args.text, data.args.schema, data.args.constructors)
